...
Future<void> saveEmailPasswordUser(String uid, String displayName, String nickName, String email,String gender, String phoneNumber ) async {
...
Future<List> getGYM() async{ // regresa una lista de la base de datos
  List gyms = [];
...
Future<List> getUser(String email) async {
  List userData = [];
  try {
    DocumentSnapshot queryPeople = await FirebaseFirestore.instance.collection('people').doc(email).get();
...
Future<List> getPeople() async{ // regresa una lista de la base de datos
  List people = [];
  CollectionReference collectionReferenceGyms = db.collection('gimnasios');
  QuerySnapshot queryGyms = await collectionReferenceGyms.get();
  for (var documento in queryGyms.docs) {
...
Future<List> getUserPrueba() async{ // regresa una lista de la base de datos
  List gyms = [];
  CollectionReference collectionReferenceGyms = db.collection('people').doc(user?.uid) as CollectionReference<Object?>;
  QuerySnapshot queryGyms = await collectionReferenceGyms.get();
  for (var documento in queryGyms.docs) {
    final Map<String, dynamic> data = documento.data() as Map<String, dynamic>;
    final gym = {
